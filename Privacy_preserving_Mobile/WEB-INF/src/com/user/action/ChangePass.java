				...
				if(result)
				{
					if(npass.equals(cpass))
					{
						...
						if(result)
						{
							...
							RequestDispatcher rd=request.getRequestDispatcher("/Files/jsp/changepass.jsp?no=4");
							rd.forward(request, response);
						}
						else
						{
							RequestDispatcher rd=request.getRequestDispatcher("/Files/jsp/changepass.jsp?no=3");
							rd.forward(request, response);
						}
					}
					else
					{
						RequestDispatcher rd=request.getRequestDispatcher("/Files/jsp/changepass.jsp?no=2");
						rd.forward(request, response);
					}
				}
				else
				{
					RequestDispatcher rd=request.getRequestDispatcher("/Files/jsp/changepass.jsp?no=1");
					rd.forward(request, response);
				}
			}
		}

		...
		try
		{
			...
			if(no==1)
			{
				...
				if(rs.next())
				{
					...
					RequestDispatcher rd=request.getRequestDispatcher("/Files/jsp/editprof.jsp?no=0");
					rd.forward(request, response);
				}
				else
					response.sendRedirect(request.getContextPath()+"/Files/jsp/editprof.jsp?no=1");
			...
			if(no==2)
			{
				...
				if(result)
				{
					...
					request.setAttribute("rs",rs);
					request.setAttribute("admin", name);
					RequestDispatcher rd=request.getRequestDispatcher("/Files/jsp/profile.jsp?no=0&no1=1");
					rd.forward(request, response);
				}
				else
					response.sendRedirect(request.getContextPath()+"/Files/jsp/editprof.jsp?no=1");
